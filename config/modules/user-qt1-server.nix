{ pkgs, ... }:

{
  programs.zsh.enable = true;

  users.users.qt1 = {
    isNormalUser = true;
    shell = pkgs.zsh;
    extraGroups = [ "wheel" ];
    openssh.authorizedKeys.keys = [
      "ssh-rsa  quentin@MacBook-Air-de-Quentin.local"
      "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIGsFq/Y/QJwBFQVTqvs/0+pefPxaVtlxhlRps9ntfA4k"
      "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIPFVRcxkp8wqLMReOG4sl0QG0gYpx4PkPudX/8Smjdfj qt1@nixos"
    ];
  };
}
